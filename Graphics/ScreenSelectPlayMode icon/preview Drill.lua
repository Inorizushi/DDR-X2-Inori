local t = Def.ActorFrame {
  ...
  Def.Sprite{
    Texture= ("Box/text/course 1x2.png");
...
-- Load of Music play frame --
  ...
  Def.Sprite{
    Texture="Box/title 1x3.png";
    InitCommand=cmd(pause;setstate,1;x,56;y,215);
    OnCommand=cmd(diffusealpha,0;y,215;sleep,0.2;smooth,0.2;y,156;diffusealpha,1);
    GainFocusCommand=function(self)
      self:Load(THEME:GetPathG("","ScreenSelectPlayMode icon/Box/title 1x3.png"))
      self:setstate(1)
    end;
    LoseFocusCommand=function(self)
      self:Load(THEME:GetPathG("","ScreenSelectPlayMode icon/Box/title shad 1x3.png"))
      self:setstate(1)
    end;
  };
  LoadActor( "Box/course shad" )..{
