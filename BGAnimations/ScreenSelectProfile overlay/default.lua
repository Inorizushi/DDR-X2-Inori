...
function LoadCard(cColor,cColor2,Player,IsJoinFrame)
    ...
    LoadActor("Frame1")..{
      InitCommand=cmd(y,-196;diffusealpha,0);
    ...
    LoadActor("Frame2")..{
      InitCommand=cmd(y,-92;diffusealpha,0);
      OnCommand=function(self)
        if IsJoinFrame then
          (cmd(linear,0.3;diffusealpha,0))(self);
        else
    ...
    LoadActor("Text1")..{
      Name="Text1";
      InitCommand=cmd(xy,-108,-197;diffusealpha,0);
      OnCommand=function(self)
        if IsJoinFrame then
          (cmd(linear,0.3;diffusealpha,0))(self);
        else
    ...
    LoadActor("radartag.png")..{
      InitCommand=cmd(diffusealpha,0;xy,-111,-28);
      OnCommand=function(self)
        if IsJoinFrame then
          (cmd(linear,0.3;diffusealpha,0))(self);
        else
    ...
    Def.ActorFrame{
      ...
      LoadActor("enjoyspin") .. {
        ...
        OffCommand=cmd(linear,0.2;rotationz,-360;diffusealpha,0);
   		...
   		LoadActor("enjoyword") .. {
         InitCommand=cmd(zoomy,0;y,40;diffuse,cColor2);
    ...
    LoadActor("Bar1")..{
			...
			OnCommand=cmd(linear,0.3;zoomx,1;linear,0.3;y,-278);
      OffCommand=function(self)
        if IsJoinFrame then
    ...
    LoadActor("Bar2.png")..{
			Name = "Frame";
			InitCommand=cmd(y,260;diffusealpha,0);
      OnCommand=function(self)
        if IsJoinFrame then
          ...
          self:sleep(0.7):linear(0.25):diffusealpha(1)
			...
			OffCommand=function(self)
				if IsJoinFrame then
					...
					(cmd(sleep,0.2;linear,0.2;diffusealpha,0))(self);
				end
			end;
		};
    LoadActor("Bar2Text.png")..{
			Name = "Frame";
			InitCommand=cmd(y,260;diffusealpha,0);
      OnCommand=function(self)
        if IsJoinFrame then
          ...
          self:Load(THEME:GetPathB("","ScreenSelectProfile overlay/Bar2Text.png"))
          self:sleep(0.7):linear(0.25):diffusealpha(1)
        end;
      end;
			OffCommand=function(self)
          ...
          self:Load(THEME:GetPathB("","ScreenSelectProfile overlay/Bar2Off.png"))
          self:sleep(0.2):linear(0.2):diffusealpha(0)
				end
			end;
		};
    Def.Sprite{
      ...
      InitCommand=function(self)
        self:xy(-3,-278):pause():diffusealpha(0):setstate(0)
      end;
  		OnCommand=function(self)
...
function LoadPlayerStuff(Player)
    ...
    InitCommand=cmd(x,-142;y,-94;zoomy,0;strokecolor,color("#000000");maxwidth,82);
		OnCommand=cmd(sleep,0.8;linear,0.05;diffusealpha,1;zoomy,0.3;linear,0.05;zoomy,1.2;linear,0.1;zoomy,1);
		OffCommand=cmd(linear,0.2;zoomy,0;diffusealpha,0);
	};
	...
	t[#t+1]=LoadFont("_helvetica Bold 24px") .. {
		Name = 'selectRank';
    InitCommand=cmd(x,-60;y,-60;diffuse,color("1,1,1,0");halign,0;skewx,-0.2;strokecolor,Color("Outline");maxwidth,350);
		OnCommand=function(self)
	...
	if (Player == PLAYER_1) then
		t[#t+1] = LoadActor( THEME:GetPathG("ScreenSelectProfile", "GrooveRadar" ),1,0.2,0.2,0.2,0.5,PLAYER_1,'single')..{
			Name = "GVRD1S";
      InitCommand=cmd(xy,4,92;zoom,1;diffusealpha,0;diffuse,PlayerColor(PLAYER_1));
    ...
    t[#t+1] = LoadActor( THEME:GetPathG("ScreenSelectProfile", "GrooveRadar" ),1,0.2,0.2,0.2,0.5,PLAYER_1, 'double')..{
			Name = "GVRD1D";
			InitCommand=cmd(xy,4,92;zoom,1;diffusealpha,0;diffuse,PlayerColor(PLAYER_2));
			OnCommand=cmd(sleep,0.9;linear,0.05;diffusealpha,1;);
			OffCommand=cmd(sleep,0.2;linear,0.2;diffusealpha,0);
		};
    t[#t+1]=LoadFont("_russell square 16px") .. {
...
local t = Def.ActorFrame {
	...
	children = {
		Def.ActorFrame {
			Name = 'P1Frame';
			InitCommand=cmd(x,SCREEN_CENTER_X-238;y,SCREEN_CENTER_Y+20;zoom,1);
