...
local function MakeRow(rownames, idx)
	return Def.ActorFrame{
		...
		LoadFont("_arial Bold 24px")..{
			InitCommand=cmd(x,-310;halign,0.4;uppercase,true;zoom,0.7;maxwidth,150);
			OnCommand=cmd(queuecommand,"Set");
			SetCommand=function(self)
				local screen = SCREENMAN:GetTopScreen();
				if screen then
					index = screen:GetCurrentRowIndex(PLAYER_1)
					CurrentRowName = screen:GetOptionRow(index):GetName()
					row = screen:GetOptionRow(idx-1);
					name = row:GetName();
					choice = row:GetChoiceInRowWithFocus(PLAYER_1)
		...
		Def.ActorFrame{
			...
			LoadActor("arrow")..{
				...
				GainFocusCommand=cmd(visible,true);
				LoseFocusCommand=cmd(visible,false);
				MenuLeftP1MessageCommand=cmd(diffuse,color("1,1,1,1");sleep,0.1;diffuse,color("#7e7e7e");sleep,0.1;diffuse,color("1,1,1,1");sleep,0.1;diffuse,color("#7e7e7e");sleep,0.1;diffuse,color("1,1,1,1");sleep,0.1;diffusealpha,0);
			};
			LoadActor("arrow")..{
			...
			LoadActor("arrow")..{
				...
				GainFocusCommand=cmd(visible,true);
				LoseFocusCommand=cmd(visible,false);
				MenuRightP1MessageCommand=cmd(diffuse,color("1,1,1,1");sleep,0.1;diffuse,color("#7e7e7e");sleep,0.1;diffuse,color("1,1,1,1");sleep,0.1;diffuse,color("#7e7e7e");sleep,0.1;diffuse,color("1,1,1,1");sleep,0.1;diffusealpha,0);
			};
		};
		Def.Sprite{
			Name="Row Name";
...
t[#t+1] =	Def.ActorFrame{
	...
	LoadFont("_helveticaneuelt pro 65 md Bold 24px")..{
	   InitCommand=cmd(xy,-118,-38;maxwidth,360;zoom,0.8;halign,0;valign,0);
		BeginCommand=cmd(queuecommand,"SetP1");
	 ...
	 Def.Sprite{
		 ...
		 SetCommand=function(self)
			 ...
			 if screen then
		 		...
		 		if CurrentRowName == "Exit" then
					self:Load(THEME:GetPathB("","ScreenPlayerOptions decorations/1P_WAIT.png"));
					self:visible(true)
				else
					self:visible(false)
				end;
		...
		OffCommand=function(self)
			self:Load(THEME:GetPathB("","ScreenPlayerOptions decorations/1P_OK.png"));
			self:visible(true)
		end;
		MenuLeftP1MessageCommand=cmd(playcommand,"Set");
		MenuRightP1MessageCommand=cmd(playcommand,"Set");
		MenuUpP1MessageCommand=cmd(playcommand,"Set");
		MenuDownP1MessageCommand=cmd(playcommand,"Set");
		MenuStartP1MessageCommand=cmd(playcommand,"Set");
	};
};
